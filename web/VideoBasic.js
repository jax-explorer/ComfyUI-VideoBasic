...
const convDict = {
    VideoBasicLoadImages : ["directory", null, "image_load_cap", "skip_first_images", "select_every_nth"],
    VideoBasicLoadImagesPath : ["directory", "image_load_cap", "skip_first_images", "select_every_nth"],
    VideoBasicVideoSave : ["frame_rate", "loop_count", "filename_prefix", "format", "pingpong", "save_image"],
    VideoBasicLoadVideo : ["video", "force_rate", "force_size", "frame_load_cap", "skip_first_frames", "select_every_nth"],
    VideoBasicLoadVideoPath : ["video", "force_rate", "force_size", "frame_load_cap", "skip_first_frames", "select_every_nth"],
};
const renameDict  = {VideoBasicVideoSave : {save_output : "save_image"}}
...
function initHelpDOM() {
    ...
    <style id="scroll-properties">
            * {
            ...
            .VideoBasicloopedvideo::-webkit-media-controls-mute-button {
                display:none;
            }
            .VideoBasicloopedvideo::-webkit-media-controls-fullscreen-button {
                display:none;
            }
        </style>
    `
    ...
    helpDOM.selectHelp = function(name, value) {
        ...
        function collapseUnlessMatch(items,t) {
            ...
            window.scrollTo(0,0)
            for (let i of items.querySelectorAll('.VideoBasiccollapse')) {
                if (i.contains(match)) {
                    setCollapse(i, false)
                } else {
                    setCollapse(i, true)
            ...
            if (linfo.type == "VideoBasicAUDIO") {
                ...
                if (inputDef?.required) {
                    for (let k in inputDef.required) {
                        if (inputDef.required[k][0] == "VideoBasicAUDIO") {
                            has_migrated = false
                            break
                        }
                    }
                ...
                if (has_migrated &&inputDef?.optional) {
                    for (let k in inputDef.optional) {
                        if (inputDef.optional[k][0] == "VideoBasicAUDIO") {
                            has_migrated = false
                            break
                        }
                    }
                ...
                if (!has_migrated) {
                    ...
                    app.ui.dialog.show("This workflow contains one or more nodes which use the old VideoBasicAUDIO format. They have been highlighted in red. An AudioToVHSAudio node must be added to convert to this legacy format")
                    tnode.bgcolor = "#C00"
                }
            }
        }
...
function cloneType(nodeType, nodeData) {
    nodeData.output[0] = "VideoBasicDUMMY_NONE"
    chainCallback(nodeType.prototype, "onConnectionsChange", function(contype, slot, iscon, linf) {
        if (contype == LiteGraph.INPUT && slot == 0) {
            let new_type = "VideoBasicDUMMY_NONE"
            if (iscon && linf) {
...
function addVideoPreview(nodeType) {
    chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        previewWidget.parentEl.className = "VideoBasicpreview";
...
function addPreviewOptions(nodeType) {
    chainCallback(nodeType.prototype, "getExtraMenuOptions", function(_, options) {
        ...
        optNew.push({content: "Sync preview", callback: () => {
            ...
            for (let p of document.getElementsByClassName("VideoBasicpreview")) {
...
function searchBox(event, [x,y], node) {
    ...
    var timeout = null;
    let last_path = null;
    let extensions = pathWidget.options.VideoBasicpath_extensions
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        if (nodeData?.name == "VideoBasicLoadImages") {
        ...
        } else if (nodeData?.name == "VideoBasicLoadImagesPath") {
            addUploadWidget(nodeType, nodeData, "directory", "folder");
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
                const pathWidget = this.widgets.find((w) => w.name === "directory");
                chainCallback(pathWidget, "callback", (value) => {
            ...
            addLoadImagesCommon(nodeType, nodeData);
        } else if (nodeData?.name == "VideoBasicLoadVideo") {
        ...
        } else if (nodeData?.name == "VideoBasicLoadAudioUpload") {
            addUploadWidget(nodeType, nodeData, "audio", "audio");
            applyVHSAudioLinksFix(nodeType, nodeData, 0)
        } else if (nodeData?.name == "VideoBasicLoadAudio"){
            applyVHSAudioLinksFix(nodeType, nodeData, 0)
        } else if (nodeData?.name =="VideoBasicLoadVideoPath") {
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
                const pathWidget = this.widgets.find((w) => w.name === "video");
                chainCallback(pathWidget, "callback", (value) => {
    ...
    async loadedGraphNode(node) {
        ...
        if (node.type?.startsWith("VideoBasic") && node.inputs) {
    ...
    async setup() {
        ...
        let graphToPrompt = async function() {
            ...
            for (let n of app.graph._nodes) {
                if (n?.type?.startsWith('VideoBasicLoadVideo')) {
